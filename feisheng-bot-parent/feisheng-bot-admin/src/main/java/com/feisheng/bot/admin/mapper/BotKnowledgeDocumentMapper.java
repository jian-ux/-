...
public interface BotKnowledgeDocumentMapper extends BaseMapper<BotKnowledgeDocument> {
    ...
    @Update("UPDATE bot_knowledge_document SET publish_status='PUBLISHED', published_at=#{publishedAt}, effective_from=#{effectiveFrom}, effective_to=NULL, supersedes_document_id=#{supersedesId} WHERE id=#{targetId} AND publish_status='DRAFT' AND deleted=0")
    int publishDraftWithSupersedesGuarded(@Param("targetId") Long targetId, @Param("publishedAt") Date publishedAt,
