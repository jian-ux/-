...
SET @column_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='bot_knowledge_migration_job' AND COLUMN_NAME='review_reason');
SET @ddl = IF(@column_exists = 0,
    'ALTER TABLE bot_knowledge_migration_job ADD COLUMN review_reason VARCHAR(500) NULL AFTER reviewed_at',
    'SELECT 1');
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @column_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='bot_knowledge_migration_job' AND COLUMN_NAME='review_audit_json');
SET @ddl = IF(@column_exists = 0,
    'ALTER TABLE bot_knowledge_migration_job ADD COLUMN review_audit_json JSON NULL AFTER review_reason',
    'SELECT 1');
PREPARE stmt FROM @ddl; EXECUTE stmt; DEALLOCATE PREPARE stmt;
